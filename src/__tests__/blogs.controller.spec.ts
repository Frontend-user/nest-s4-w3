...
describe("Blogs", () => {
  ...
  describe("BLogs", () => {
    ...
    it(`CREATE BLOG`, async () => {
      const response = await testManager.createBlog("     ");
      expect(response).toEqual("s");
    });
    it(`CREATE BLOG`, async () => {
      const response = await testManager.createWrongBodyBlog();
      expect(response).toEqual("s");
    });
    // it(`CREATE BLOG`, async () => {
...
describe("Posts LIKESTATUS", () => {
    ...
    it(`CREATE POST BY BLOG ID`, async () => {
      postId_One = await testManager.craetePostByBlogId(blogId_One);
      console.log(postId_One, "postId_One");
      expect(postId_One).toEqual("s");
    });
  });
  // let blog_1: any;
  // let post_1: any;
